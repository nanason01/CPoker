...
public:
    ...
    GameTree()
        ...
        first_action(6, { 0.01, 0.0 }), first_bet_response(6, { 0.01, 0.0 }) {}
    ...
    Utility train_bet_response(int card_1, int card_2, float prob, bool print = false) {
        ...
        float call = std::max(std::min(sum_call_regr / sum_regr, 100.0F), 0.0F);
        float fold = std::max(std::min(sum_fold_regr / sum_regr, 100.0F), 0.0F);
    ...
    Utility train_first_action(int card_1, int card_2, float prob, bool print = false) {
        ...
        float bet = std::max(std::min(sum_bet_regr / sum_regret, 100.0F), 0.0F);
        float check = std::max(std::min(sum_check_regr / sum_regret, 100.0F), 0.0F);
        ...
        first_action[(card_1 - 1) * 2 + 1].sum_regret += prob * check;
    ...
    void print() {
        ...
        cout << "2:\n\tBet:" <<
            first_action[2].sum_strats / sum_reg2 << "\n\tCheck:" <<
            first_action[3].sum_strats / sum_reg2 << "\n";
        cout << "3:\n\tBet:" <<
        ...
        cout << "1:\n\tCall:" <<
            first_bet_response[0].sum_strats / sum_reg1 << "\n\tFold:" <<
            first_bet_response[1].sum_strats / sum_reg1 << "\n";
        cout << "2:\n\tCall:" <<
            first_bet_response[2].sum_strats / sum_reg2 << "\n\tFold:" <<
            first_bet_response[3].sum_strats / sum_reg2 << "\n";
        cout << "3:\n\tCall:" <<
            first_bet_response[4].sum_strats / sum_reg3 << "\n\tFold:" <<
            first_bet_response[5].sum_strats / sum_reg3 << "\n";
    }
};
